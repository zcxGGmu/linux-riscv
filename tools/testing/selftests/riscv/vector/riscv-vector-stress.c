...
static void start_v_test(struct child_data *child, int cpu, int copy)
{
	int ret;

	ret = asprintf(&child->name, "V-TEST-%d-%d", cpu, copy);
	if (ret == -1)
		ksft_exit_fail_msg("asprintf() failed\n");

	child_start(child, "./v-test");
...
int main(int argc, char **argv)
{
	...
	// For RISC-V, we just start a fixed number of tests per CPU
	tests += cpus * 2;

	ksft_print_header();
	ksft_set_plan(tests);

	ksft_print_msg("%d CPUs\n", cpus);
	...
	for (i = 0; i < cpus; i++) {
		start_v_test(&children[num_children++], i, 0);
