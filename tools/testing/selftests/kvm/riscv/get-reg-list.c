...
#define KVM_RISCV_VECTOR_REG_NUM 37
...
static __u64 vector_regs[KVM_RISCV_VECTOR_REG_NUM + 1];
...
bool filter_reg(__u64 reg)
{
	...
	case KVM_REG_RISCV_CSR | KVM_REG_RISCV_CSR_AIA |
		KVM_REG_RISCV_CSR_AIA_REG(iprio1):
	case KVM_REG_RISCV_CSR | KVM_REG_RISCV_CSR_AIA |
		KVM_REG_RISCV_CSR_AIA_REG(iprio2):
	case KVM_REG_RISCV_CSR | KVM_REG_RISCV_CSR_AIA |
		KVM_REG_RISCV_CSR_AIA_REG(sieh):
	case KVM_REG_RISCV_CSR | KVM_REG_RISCV_CSR_AIA |
...
void finalize_vcpu(struct kvm_vcpu *vcpu, struct vcpu_reg_list *c)
{
	...
	for (int i = 0; i < KVM_RISCV_SBI_EXT_MAX; i++) {
		...
		TEST_ASSERT(!rc || (rc == -1 && errno == ENOENT),
			    "Unexpected error");
	}
	...
	if (__vcpu_has_ext(vcpu, KVM_RISCV_ISA_EXT_V)) {
		unsigned long vlenb;
		__u64 id = KVM_REG_RISCV |
				   KVM_REG_SIZE_ULONG |
				   KVM_REG_RISCV_VECTOR |
				   KVM_REG_RISCV_VECTOR_CSR_REG(vlenb);
		__vcpu_get_reg(vcpu, id, &vlenb);
		finalize_vector_regs(vector_regs, vlenb);
	}
}
...
static const char *core_id_to_str(const char *prefix, __u64 id)
{
	...
	case KVM_REG_RISCV_CORE_REG(regs.s2)... KVM_REG_RISCV_CORE_REG(
		regs.s11):
		return strdup_printf("KVM_REG_RISCV_CORE_REG(regs.s%lld)",
...
static const char *aia_csr_id_to_str(__u64 reg_off)
{
	...
	return strdup_printf("KVM_REG_RISCV_CSR_AIA | %lld /* UNKNOWN */",
			     reg_off);
}
...
static const char *fp_f_id_to_str(const char *prefix, __u64 id)
{
	...
	switch (reg_off) {
	case KVM_REG_RISCV_FP_F_REG(f[0])... KVM_REG_RISCV_FP_F_REG(f[31]):
...
static const char *fp_d_id_to_str(const char *prefix, __u64 id)
{
	...
	switch (reg_off) {
	case KVM_REG_RISCV_FP_D_REG(f[0])... KVM_REG_RISCV_FP_D_REG(f[31]):
...
static const char *vector_id_to_str(const char *prefix, __u64 id)
{
	...
	switch (reg_off) {
	case KVM_REG_RISCV_VECTOR_REG(0)... KVM_REG_RISCV_VECTOR_REG(31):
		return strdup_printf("KVM_REG_RISCV_VECTOR_REG(%lld)", reg_off);
...
static const char *isa_ext_single_id_to_str(__u64 reg_off)
{
	static const char *const kvm_isa_ext_reg_name[] = {
		KVM_ISA_EXT_ARR(A),	    KVM_ISA_EXT_ARR(C),
		KVM_ISA_EXT_ARR(D),	    KVM_ISA_EXT_ARR(F),
		KVM_ISA_EXT_ARR(H),	    KVM_ISA_EXT_ARR(I),
		KVM_ISA_EXT_ARR(M),	    KVM_ISA_EXT_ARR(V),
		KVM_ISA_EXT_ARR(SMSTATEEN), KVM_ISA_EXT_ARR(SSAIA),
		KVM_ISA_EXT_ARR(SSCOFPMF),  KVM_ISA_EXT_ARR(SSTC),
		KVM_ISA_EXT_ARR(SVINVAL),   KVM_ISA_EXT_ARR(SVNAPOT),
		KVM_ISA_EXT_ARR(SVPBMT),    KVM_ISA_EXT_ARR(ZACAS),
		KVM_ISA_EXT_ARR(ZAWRS),	    KVM_ISA_EXT_ARR(ZBA),
		KVM_ISA_EXT_ARR(ZBB),	    KVM_ISA_EXT_ARR(ZBC),
		KVM_ISA_EXT_ARR(ZBKB),	    KVM_ISA_EXT_ARR(ZBKC),
		KVM_ISA_EXT_ARR(ZBKX),	    KVM_ISA_EXT_ARR(ZBS),
		KVM_ISA_EXT_ARR(ZCA),	    KVM_ISA_EXT_ARR(ZCB),
		KVM_ISA_EXT_ARR(ZCD),	    KVM_ISA_EXT_ARR(ZCF),
		KVM_ISA_EXT_ARR(ZCMOP),	    KVM_ISA_EXT_ARR(ZFA),
		KVM_ISA_EXT_ARR(ZFH),	    KVM_ISA_EXT_ARR(ZFHMIN),
		KVM_ISA_EXT_ARR(ZICBOM),    KVM_ISA_EXT_ARR(ZICBOZ),
		KVM_ISA_EXT_ARR(ZICNTR),    KVM_ISA_EXT_ARR(ZICOND),
		KVM_ISA_EXT_ARR(ZICSR),	    KVM_ISA_EXT_ARR(ZIFENCEI),
		KVM_ISA_EXT_ARR(ZIHINTNTL), KVM_ISA_EXT_ARR(ZIHINTPAUSE),
		KVM_ISA_EXT_ARR(ZIHPM),	    KVM_ISA_EXT_ARR(ZIMOP),
		KVM_ISA_EXT_ARR(ZKND),	    KVM_ISA_EXT_ARR(ZKNE),
		KVM_ISA_EXT_ARR(ZKNH),	    KVM_ISA_EXT_ARR(ZKR),
		KVM_ISA_EXT_ARR(ZKSED),	    KVM_ISA_EXT_ARR(ZKSH),
		KVM_ISA_EXT_ARR(ZKT),	    KVM_ISA_EXT_ARR(ZTSO),
		KVM_ISA_EXT_ARR(ZVBB),	    KVM_ISA_EXT_ARR(ZVBC),
		KVM_ISA_EXT_ARR(ZVFH),	    KVM_ISA_EXT_ARR(ZVFHMIN),
		KVM_ISA_EXT_ARR(ZVKB),	    KVM_ISA_EXT_ARR(ZVKG),
		KVM_ISA_EXT_ARR(ZVKNED),    KVM_ISA_EXT_ARR(ZVKNHA),
		KVM_ISA_EXT_ARR(ZVKNHB),    KVM_ISA_EXT_ARR(ZVKSED),
		KVM_ISA_EXT_ARR(ZVKSH),	    KVM_ISA_EXT_ARR(ZVKT),
	};

	if (reg_off >= ARRAY_SIZE(kvm_isa_ext_reg_name))
		return strdup_printf(
			"KVM_REG_RISCV_ISA_SINGLE | %lld /* UNKNOWN */",
			reg_off);
...
#define KVM_SBI_EXT_ARR(ext) [ext] = "KVM_REG_RISCV_SBI_SINGLE | " #ext
...
static const char *sbi_ext_single_id_to_str(__u64 reg_off)
{
	...
	if (reg_off >= ARRAY_SIZE(kvm_sbi_ext_reg_name))
		return strdup_printf(
			"KVM_REG_RISCV_SBI_SINGLE | %lld /* UNKNOWN */",
			reg_off);
...
static const char *sbi_ext_multi_id_to_str(__u64 reg_subtype, __u64 reg_off)
{
	...
	case KVM_REG_RISCV_SBI_MULTI_EN:
		return strdup_printf("KVM_REG_RISCV_SBI_MULTI_EN | %lld%s",
				     reg_off, unknown);
	case KVM_REG_RISCV_SBI_MULTI_DIS:
		return strdup_printf("KVM_REG_RISCV_SBI_MULTI_DIS | %lld%s",
				     reg_off, unknown);
	}
...
#define SUBLIST_BASE                                       \
	{                                                  \
		...
		.skips_set = base_skips_set,               \
		.skips_set_n = ARRAY_SIZE(base_skips_set), \
	}
#define SUBLIST_SBI_BASE                              \
	{                                             \
		...
		.regs = sbi_base_regs,                \
		.regs_n = ARRAY_SIZE(sbi_base_regs),  \
	}
#define SUBLIST_SBI_STA                               \
	{                                             \
		...
		.regs = sbi_sta_regs,                 \
		.regs_n = ARRAY_SIZE(sbi_sta_regs),   \
	}
#define SUBLIST_ZICBOM                               \
	{                                            \
		"zicbom",                            \
		.feature = KVM_RISCV_ISA_EXT_ZICBOM, \
		.regs = zicbom_regs,                 \
...
#define SUBLIST_ZICBOZ                               \
	{                                            \
		"zicboz",                            \
		.feature = KVM_RISCV_ISA_EXT_ZICBOZ, \
		.regs = zicboz_regs,                 \
...
#define SUBLIST_AIA                                 \
	{                                           \
		"aia",                              \
		.feature = KVM_RISCV_ISA_EXT_SSAIA, \
		.regs = aia_regs,                   \
...
#define SUBLIST_SMSTATEEN                               \
	{                                               \
		"smstateen",                            \
		.feature = KVM_RISCV_ISA_EXT_SMSTATEEN, \
		.regs = smstateen_regs,                 \
...
#define SUBLIST_FP_D                             \
	{                                        \
		"fp_d",                          \
		.feature = KVM_RISCV_ISA_EXT_D,  \
		.regs = fp_d_regs,               \
		.regs_n = ARRAY_SIZE(fp_d_regs), \
	}
#define SUBLIST_VECTOR                             \
	{                                          \
		"vector",                          \
		.feature = KVM_RISCV_ISA_EXT_V,    \
		.regs = vector_regs,               \
...
#define KVM_ISA_EXT_SIMPLE_CONFIG(ext, extu)                                 \
	static __u64 regs_##ext[] = {                                        \
		KVM_REG_RISCV | KVM_REG_SIZE_ULONG | KVM_REG_RISCV_ISA_EXT | \
			KVM_REG_RISCV_ISA_SINGLE | KVM_RISCV_ISA_EXT_##extu, \
	};                                                                   \
...
#define KVM_SBI_EXT_SIMPLE_CONFIG(ext, extu)                                 \
	static __u64 regs_sbi_##ext[] = {                                    \
		KVM_REG_RISCV | KVM_REG_SIZE_ULONG | KVM_REG_RISCV_SBI_EXT | \
			KVM_REG_RISCV_SBI_SINGLE | KVM_RISCV_SBI_EXT_##extu, \
	};                                                                   \
...
KVM_ISA_EXT_SUBLIST_CONFIG(vector, VECTOR);
...
struct vcpu_reg_list *vcpu_configs[] = {
	&config_sbi_base,    &config_sbi_sta,  &config_sbi_pmu,
	&config_sbi_dbcn,    &config_aia,      &config_fp_f,
	&config_fp_d,	     &config_vector,   &config_h,
	&config_smstateen,   &config_sscofpmf, &config_sstc,
	&config_svinval,     &config_svnapot,  &config_svpbmt,
	&config_zacas,	     &config_zawrs,    &config_zba,
	&config_zbb,	     &config_zbc,      &config_zbkb,
	&config_zbkc,	     &config_zbkx,     &config_zbs,
	&config_zca,	     &config_zcb,      &config_zcd,
	&config_zcf,	     &config_zcmop,    &config_zfa,
	&config_zfh,	     &config_zfhmin,   &config_zicbom,
	&config_zicboz,	     &config_zicntr,   &config_zicond,
	&config_zicsr,	     &config_zifencei, &config_zihintntl,
	&config_zihintpause, &config_zihpm,    &config_zimop,
	&config_zknd,	     &config_zkne,     &config_zknh,
	&config_zkr,	     &config_zksed,    &config_zksh,
	&config_zkt,	     &config_ztso,     &config_zvbb,
	&config_zvbc,	     &config_zvfh,     &config_zvfhmin,
	&config_zvkb,	     &config_zvkg,     &config_zvkned,
	&config_zvknha,	     &config_zvknhb,   &config_zvksed,
	&config_zvksh,	     &config_zvkt,
};
